...
#define BINARY_MAGIC 23

CliPort master_slave_cli = {
    USART_MASTERSLAVE,
    "",
    "",
    "",
...
const PConfig parameters[] PROGMEM = {
  {FST("mct"), FST("motor control type: 0=power 1=speed 3=angle"), pt_int, 0, &control_type, nullptr},
...
void cliRun(CliPort* cli) {
  while (cli->rx_count) {
    ...
    if (cli->command[0] == BINARY_MAGIC) { // Binary command
        if (cli->command_index > 3 && cli->command_index == (cli->command[2] + 4)){
            cli->command_index = 0;
            cliExecuteBinary(cli, cli->command);
        }
    } else { // ASCII command
        if (c == '\n' || c == '\r') { 
    ...
    if (cli->command_index > COMMAND_BUFFER_SIZE-1) {
        cli->command_index = COMMAND_BUFFER_SIZE-1;
    }
  }
}

CliResult cliExecuteBinary(CliPort* cli, const char* cmd) {
  DEBUG_print(FST("Binary: %d\n", cli->command[1]));
  cli->error = nullptr;
  if (cmd[0] != BINARY_MAGIC) {
    return CR_BAD_MAGIC;    
  }
  size_t pn = cmd[1];
  if (pn >= MAX_PARAMETER) {
...
void proxy_value(size_t pn, uint32_t value) {
    uint8_t buffer[8];
    buffer[0] = BINARY_MAGIC;
    buffer[1] = pn;
    ...
    SendBuffer(USART1, buffer, sizeof(buffer));
}

const char* getParameter(CliPort* cli, const PConfig* p) {
  switch ( p->type ) {
    case pt_bool: {
...
const char* controlParameter(CliPort* cli, int target, size_t pn, const char* cmd) {
  const PConfig* p = &parameters[pn];
  if ( cmd[0] == '\0' ) { return getParameter(cli, p); }
  switch ( p->type ) {
    case pt_bool: {
      bool value = 0;
